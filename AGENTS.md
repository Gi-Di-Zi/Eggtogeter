...
4) 테스트를 수행했다면:
   - `@/docs/TestData/YYYY-MM-DD_<task-slug>.md` 기록
5) 규칙/스킬/설정 스냅샷 파일이 변경됐다면:
   - `python scripts/notion_sync_settings.py` 실행으로 Notion 스냅샷 갱신

### Notion 운영 보조 규칙
- 새 환경에서 로컬 규칙/스킬이 부족하면:
  - `python scripts/notion_bootstrap_pull.py`로 Notion 최신 스냅샷 번들을 추출한다.
- 변경 감시 기반 반자동 동기화가 필요하면:
  - `python scripts/notion_sync_watch.py` 사용
