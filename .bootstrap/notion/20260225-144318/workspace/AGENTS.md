...
### 4.1 기본 스택 고정
- Frontend: **Vite + TypeScript + Vue 3**
  - Vue는 **Composition API**
  - `.vue` 기본 템플릿은 **`<script setup lang="ts">`**
- Backend: **Spring Boot**
  - 계층: Controller / Service / Repository(또는 Adapter)
...
4) 테스트를 수행했다면: `@/docs/TestData/YYYY-MM-DD_<task-REDACTED>.md`
...
### 6.3 Antigravity 위임 유지
- E2E/UI 실제 화면 검증
- 시각 검증(스크린샷 중심 QA)
- 이미지/에셋 생성·가공

### 6.4 Playwright/MCP 정책
